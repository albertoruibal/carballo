...
public class Pgn {
	...
	public String getPgn(Board b, String whiteName, String blackName) {
		...
		if (whiteName == null || "".equals(whiteName)) whiteName = "?";
		if (blackName == null || "".equals(blackName)) blackName = "?";
		...
		switch (b.isEndGame()) {
			...
			case 99: result = "1/2-1/2"; break;
		}
		sb.append("[Result \"" + result + "\"]\n");
		if (!Board.FEN_START_POSITION.equals(b.initialFen)) {
			sb.append("[FEN \""+b.initialFen+"\"]\n");
		}
		sb.append("[PlyCount \""+b.moveNumber+"\"]\n");
		sb.append("\n");
		
		StringBuffer line = new StringBuffer(); 
		
		for (int i=0; i<b.moveNumber; i++) {
			line.append(" ");
			if ((i&1) == 0) {
				line.append((i>>>1) + 1);
				line.append(".");
		...
		try {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (line.indexOf("[") == 0) {
					...
					if ("event".equals(headerName)) event = headerValue;
					if ("round".equals(headerName)) round = headerValue;
					if ("site".equals(headerName)) site = headerValue;
					if ("date".equals(headerName)) date = headerValue;
					if ("white".equals(headerName)) white = headerValue;
					if ("black".equals(headerName)) black = headerValue;
					if ("whiteelo".equals(headerName)) whiteElo = headerValue;
					if ("blackelo".equals(headerName)) blackElo = headerValue;
					if ("result".equals(headerName)) result = headerValue;
					if ("fen".equals(headerName)) fenStartPosition = headerValue;
				} else {
					movesSb.append(line);
					movesSb.append(" ");
				}
					...
					if (counter == gameNumber) {
						...
						try {
							while (true) {
								...
								pgnSb.append("\n");
								line = br.readLine();
								if (line == null || line.indexOf("[Event ") == 0) break;
							}
						} catch (IOException e) {}
