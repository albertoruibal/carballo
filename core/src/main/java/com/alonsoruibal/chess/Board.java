...
public class Board {
	...
	public static final String FEN_START_POSITION = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 0";
	...
	public void setPieceAt(long square, char piece) {	
		...
		switch (Character.toLowerCase(piece)) {
			case 'p' : pawns   |= square; break;
			case 'q' : queens  |= square; break; 
			case 'r' : rooks   |= square; break;
			case 'b' : bishops |= square; break; 
			case 'n' : knights |= square; break;
			case 'k' : kings   |= square; break;
		}
	}
	...
	public void setFen(String fen) {
        ...
        if (tokens.length > 2) {
            ...
            if (tokens.length > 3) {
	            ...
	            if (tokens.length > 4) {
					try {
						fiftyMovesRule = Integer.parseInt(tokens[4]);
					} catch (Exception e) {
						fiftyMovesRule = 0;
					}	
//					if (st.hasMoreTokens()) {
...
//					}
	...
	private void verify() {
		...
		// Verify castling
		if (getWhiteKingsideCastling()  && ((whites & kings & 0x08) == 0 || (whites & rooks & 0x01) == 0)) flags |= FLAG_WHITE_DISABLE_KINGSIDE_CASTLING;   
		if (getWhiteQueensideCastling() && ((whites & kings & 0x08) == 0 || (whites & rooks & 0x80) == 0)) flags |= FLAG_WHITE_DISABLE_QUEENSIDE_CASTLING;   
		if (getBlackKingsideCastling()  && ((blacks & kings & 0x0800000000000000L) == 0 || (blacks & rooks & 0x0100000000000000L) == 0)) flags |= FLAG_BLACK_DISABLE_KINGSIDE_CASTLING;   
		if (getBlackQueensideCastling() && ((blacks & kings & 0x0800000000000000L) == 0 || (blacks & rooks & 0x8000000000000000L) == 0)) flags |= FLAG_BLACK_DISABLE_QUEENSIDE_CASTLING;   
	}
...
//		sb.append(" " +getWhiteKingsideCastling()+getWhiteQueensideCastling()+getBlackKingsideCastling()+getBlackQueensideCastling());
	...
	private void resetHistory() {
		...
		for (int i = 0; i < MAX_MOVES; i++) Arrays.fill(keyHistory[i], 0);
	...
	public boolean getLastMoveIsRecapture() {
		if (moveNumber > 1
			&& Move.getToIndex(moveHistory[moveNumber-1]) == Move.getToIndex(moveHistory[moveNumber-2])	
			&& capturedPieces[moveNumber-1] != '.'
			&& capturedPieces[moveNumber-2] != '.') {
			...
			if (captured1 == 'n') captured1 = 'b'; // Converts knights in bishops
			if (captured2 == 'n') captured2 = 'b';
			if (captured1 == captured2) return true;
		}
		return false;
	}
...
//		logger.debug("Before move: \n" + toString() + "\n " + Move.toStringExt(move));
		...
		long moveMask = from | to; // Move is as easy as xor with this mask (exceptions are in captures, promotions and passant captures)
...
//		}
	...
	public boolean isDraw() {
		if (fiftyMovesRule >= 100) return true; 
		int repetitions = 0;
...
//			logger.debug("movenumber="+i+" key0=" + keyHistory[i][0] + " " + " key1=" + keyHistory[i][1] + " Repetitions="+repetitions);
		...
		if (pawns == 0 && rooks == 0 && queens == 0 && bishops == 0 && knights == 0) return true;
		return false;
	}
	...
	public int see(int move) {
		...
		if ((to & knights) != 0) pieceCaptured = Move.KNIGHT;
		else if ((to & bishops) != 0) pieceCaptured = Move.BISHOP;
		else if ((to & rooks) != 0) pieceCaptured = Move.ROOK;
		else if ((to & queens) != 0) pieceCaptured = Move.QUEEN;
		else if (Move.isCapture(move)) pieceCaptured = Move.PAWN;
	...
	public int see(int fromIndex, int toIndex, int pieceMoved, int targetPiece)
	{
	   ...
	   do {
	      ...
	      seeGain[d]  = SEE_PIECE_VALUES[pieceMoved] - seeGain[d-1]; // speculative store, if defended
	...
	public boolean isPassedPawn(int index) {
		...
		if ((whites & square) != 0) {
			return ((BitboardUtils.COLUMN[column] | BitboardUtils.COLUMNS_ADJACENTS[column]) & BitboardUtils.RANKS_UPWARDS[rank]
				& pawns & blacks) == 0;
		} else if ((blacks & square) != 0) {
			return ((BitboardUtils.COLUMN[column] | BitboardUtils.COLUMNS_ADJACENTS[column]) & BitboardUtils.RANKS_DOWNWARDS[rank]
				& pawns & whites) == 0;
		}
		return false;
	}
	...
	public boolean isMoveLegal(int move) {
		...
		for (int i = 0; i < legalMoveCount; i++) {
			...
			if (move == legalMoves[i]) moveOk = true;
		}
		return moveOk;
	}
