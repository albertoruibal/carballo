...
public class ExperimentalEvaluator extends Evaluator {
	...
	// Tempo
	private static final int TEMPO = oe(15, 5); // Add to moving side score
	...
	private final boolean debugPawns = false;
		...
		for (int index = 0; index < 64; index++) {
			if ((square & all) != 0) {
				...
				if ((square & board.pawns) != 0) {
					...
					// Note: absolute pins are already detected by AttacksInfo.pinnedPieces and
					// scored in evalAttacks(); the per-piece x-ray scan that used to live here
					// double-counted pinned rooks/queens.
