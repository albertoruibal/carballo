...
//						pawnStructure[color] += PAWN_BACKWARD;
					...
					if (((BitboardUtils.COLUMN[column] | BitboardUtils.COLUMNS_ADJACENTS[column]) &
						...
						if ((square & pawnAttacks[color]) != 0) passedPawns[color] += PAWN_PASSER_SUPPORT[(isWhite ? rank : 7-rank)];
...
//					}
					...
					if ((BISHOP_TRAPPING[index] & board.pawns & others) != 0) mobility[color] += BISHOP_TRAPPED; 
