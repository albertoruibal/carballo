...
namespace Com.Alonsoruibal.Chess.Evaluation
{
	...
	/// </summary>
	/// <author>rui</author>
	public class ExperimentalEvaluatorNew : Evaluator
	{
		/// <summary>Logger for this class</summary>
		private static readonly Logger logger = Logger.GetLogger("ExperimentalEvaluatorNew"
			);
		...
		public ExperimentalEvaluatorNew(Config config)
		{
			this.config = config;
		}
		...
		private static readonly int ROOK_FILE_OPEN_NO_MG = Oe(20, 10);

		private static readonly int ROOK_FILE_OPEN_MG_NP = Oe(10, 0);

		private static readonly int ROOK_FILE_OPEN_MG_P = Oe(15, 5);

		private static readonly int ROOK_FILE_SEMIOPEN = Oe(3, 6);

		private static readonly int ROOK_FILE_SEMIOPEN_BP = Oe(15, 5);

		private static readonly int ROOK_FILE_SEMIOPEN_K = Oe(3, 6);
		...
		private static readonly int[][] PawnFile = new int[][] { new int[] { -20, -8, -2, 
			5, 5, -2, -8, -20 }, new int[] { -4, -6, -8, -10, -10, -8, -6, -4 } };

		private static readonly int[][] PawnRank = new int[][] { new int[] { 0, -3, -2, -
			1, 1, 2, 3, 0 }, new int[] { 0, -3, -3, -2, -1, 0, 2, 0 } };

		private static readonly int[][] KnightFile = new int[][] { new int[] { -26, -10, 
		...
		private static readonly int[][] RookFile = new int[][] { new int[] { -4, 0, 4, 8, 
			8, 4, 0, -4 }, new int[] { 0, 0, 0, 0, 0, 0, 0, 0 } };

		private static readonly int[][] RookRank = new int[][] { new int[] { 0, 0, 0, 0, 
			0, 0, 0, 0 }, new int[] { 0, 0, 0, 0, 1, 1, 1, -2 } };

		private static readonly int[][] QueenFile = new int[][] { new int[] { -2, 0, 1, 2
		...
		private static readonly int[][] KingFile = new int[][] { new int[] { 40, 45, 15, 
		...
		static ExperimentalEvaluatorNew()
		{
			...
			// Attacks queen
			// No pawns in rook file and no minor guarded
			// No pawns in rook file and minor guarded, my pawn can attack
			// No pawns in rook file and minor guarded, my pawn can attack
			...
			for (i = 0; i < 64; i++)
			{
				int rank = i >> 3;
				int file = 7 - i & 7;
				int d = (((file - rank) >= 0) ? (file - rank) : -(file - rank));
				int e = (((file + rank - 7) >= 0) ? (file + rank - 7) : -(file + rank - 7));
				pawnIndexValue[i] = Oe(PawnFile[OPENING][file] + PawnRank[OPENING][rank], PawnFile
					[ENDGAME][file] + PawnRank[ENDGAME][rank]);
				knightIndexValue[i] = Oe(KnightFile[OPENING][file] + KnightRank[OPENING][rank] + 
					KnightLine[OPENING][d] + KnightLine[OPENING][e], KnightFile[ENDGAME][file] + KnightRank
					[ENDGAME][rank] + KnightLine[ENDGAME][d] + KnightLine[ENDGAME][e]);
				...
				rookIndexValue[i] = Oe(RookFile[OPENING][file] + RookRank[OPENING][rank], RookFile
					[ENDGAME][file] + RookRank[ENDGAME][rank]);
				queenIndexValue[i] = Oe(QueenFile[OPENING][file] + QueenRank[OPENING][rank] + QueenLine
					[OPENING][d] + QueenLine[OPENING][e], QueenFile[ENDGAME][file] + QueenRank[ENDGAME
					][rank] + QueenLine[ENDGAME][d] + QueenLine[ENDGAME][e]);
				kingIndexValue[i] = Oe(KingFile[OPENING][file] + KingRank[OPENING][rank] + KingLine
					[OPENING][d] + KingLine[OPENING][e], KingFile[ENDGAME][file] + KingRank[ENDGAME]
			...
			while (square != 0)
			{
				if ((square & all) != 0)
				{
					...
					if ((square & board.pawns) != 0)
					{
						...
						bool isolated = (BitboardUtils.COLUMNS_ADJACENTS[file] & board.pawns & mines) == 
							0;
						bool doubled = (BitboardUtils.COLUMN[file] & BitboardUtils.RANKS_FORWARD[color][rank
							] & board.pawns & mines) != 0;
						//boolean backwards = ((BitboardUtils.FILE[file] | BitboardUtils.FILES_ADJACENTS[file]) & ~BitboardUtils.RANKS_FORWARD[color][rank] & board.pawns & mines) == 0;
						bool weak = !isolated && (pawnAttacks[color] & square) == 0 && pcsqIndex >= 24;
						// not defended is weak
						if (weak)
						{
							// Can be defended advancing one pawn
							auxLong = (isWhite ? BitboardAttacks.pawnDownwards[color] : BitboardAttacks.pawnUpwards
								[color]) & ~otherPawnAttacks & ~all;
							while (auxLong != 0)
							{
							...
							if (weak)
							{
								...
								auxLong = (isWhite ? square << 8 : (long)(((ulong)square) >> 8)) & ~otherPawnAttacks
									 & ~all;
								if (auxLong != 0)
								{
									...
									else
									{
										...
										{
											auxLong = (isWhite ? square << 16 : (long)(((ulong)square) >> 16)) & ~otherPawnAttacks
												 & ~all;
											if ((auxLong & pawnAttacks[color]) != 0)
											{
												weak = false;
						...
						//					}
						// No pawns in front
						if ((BitboardUtils.COLUMN[file] & BitboardUtils.RANKS_FORWARD[color][rank] & board
							.pawns) == 0)
						{
							if (doubled)
							{
						...
						//if (backwards && (BitboardUtils.FILE[file] & BitboardUtils.RANKS_FORWARD[color][rank] & otherPawnAttacks) !=0)
						//	pawnStructure[color] += PAWN_BACKWARDS;
						// Passed Pawns
						if (((BitboardUtils.COLUMN[file] | BitboardUtils.COLUMNS_ADJACENTS[file]) & BitboardUtils
							.RANKS_FORWARD[color][rank] & board.pawns & others) == 0)
						{
							...
							if ((BitboardUtils.ROWS_LEFT[file] & board.pawns & others) == 0 && (BitboardUtils
								.ROWS_RIGHT[file] & board.pawns & others) == 0)
							{
								passedPawns[color] += PAWN_PASSER_OUTSIDE[(isWhite ? rank : 7 - rank)];
							}
							// Dynamic part
							auxLong = BitboardUtils.COLUMN[file] & BitboardUtils.RANKS_FORWARD[color][rank];
							if ((auxLong & mines) == 0)
							{
							...
							if ((BitboardUtils.COLUMN[file] & BitboardUtils.RANKS_BACKWARD[color][rank] & board
								.rooks & mines) != 0)
							{
